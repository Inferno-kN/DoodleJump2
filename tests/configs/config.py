...
def resource_path(*parts: str) -> str:
    ...
    if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
        ...
        # configs.py лежит в src/refactoring/configs,
        # поэтому поднимаемся на один уровень — в src/refactoring
        base = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
    return os.path.normpath(os.path.join(base, *parts))

# Теперь ресурсы лежат в src/refactoring/rsrs
doodler         = resource_path('resources', 'imgs', 'doodle.png')
simple_platform = resource_path('resources', 'imgs', 'platform.png')
broken_platform = resource_path('resources', 'imgs', 'broken_platform.png')
background      = resource_path('resources', 'fnts', 'background.png')
